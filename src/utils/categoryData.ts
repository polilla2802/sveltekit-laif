export const categoryData: Record<
	string,
	{ title: string, bgColor: string; mainVideoSrc: string; altVideoSrc: string; label: string; icon: string }
> = {
	laif: {
		title: 'Laif',
		bgColor: '#323119',
		mainVideoSrc:
			'https://www.twitch.tv/hecho_por_hugo',
		altVideoSrc:
			'https://www.twitch.tv/hecho_por_hugo',
		label:
			'Transmisión en vivo',
		icon: '/logos/laif-logo.png'
	},
	bienes: {
		title: 'Bienes y Raices',
		bgColor: '#E35836',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fbienesyraices.mp4?alt=media&token=6f90b453-3e74-49d2-9cd3-8b9195a0b842',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fbienesyraices.mp4?alt=media&token=6f90b453-3e74-49d2-9cd3-8b9195a0b842',
		label:
			'La sección de "Bienes y Raíces" de nuestro canal te mantiene informado sobre las últimas tendencias del mercado inmobiliario, ofreciendo consejos de expertos, análisis de propiedades y oportunidades de inversión. Ya sea que busques tu hogar ideal o quieras conocer las mejores ofertas, esta sección te brinda todo lo necesario para tomar decisiones inteligentes en el mundo inmobiliario.',
		icon: '/icons/icon-bienes.png'
	},
	vida: {
		title: 'Estilo de Vida',
		bgColor: '#9AA9E6',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Festilo%20de%20vida.mp4?alt=media&token=7ef906bc-a94d-45a9-9ec6-de0ec578de19',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Festilo%20de%20vida.mp4?alt=media&token=7ef906bc-a94d-45a9-9ec6-de0ec578de19',
		label:
			'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
		icon: '/icons/icon-vida.png'
	},
	industria: {
		title: 'Industria',
		bgColor: '#F5D579',
		mainVideoSrc:
			'https://www.youtube.com/live/p2AzyIEuFak',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Findustria.mp4?alt=media&token=79de4c07-29e0-405a-adfd-13b0903130fa',
		label:
			'La sección de "Industria" de nuestro canal turístico explora la fascinante intersección entre el desarrollo humano y el entorno natural. A través de reportajes sobre proyectos sostenibles, ecoturismo y la innovación en la industria verde, mostramos cómo las actividades humanas pueden coexistir en armonía con la naturaleza, destacando destinos que promueven un turismo responsable y respetuoso con el medio ambiente.',
		icon: '/icons/icon-industria.png'
	},
	destinos: {
		title: 'Destinos',
		bgColor: '#476C87',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fdestinos.mp4?alt=media&token=62b0d520-761b-476b-a1c6-feafff808500',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fdestinos-portada.mp4?alt=media&token=1cc37946-9ad8-40a7-b398-3c6c02e64b00',
		label:
			'Descubre los destinos más fascinantes del mundo. En esta sección, te llevamos a explorar los rincones más hermosos y exóticos, con recomendaciones de lugares imperdibles, actividades únicas y cultura local',
		icon: '/icons/icon-destinos.png'
	},
	explora: {
		title: 'Explora',
		bgColor: '#B4ACCC',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fexplora.mp4?alt=media&token=bba61a77-f216-4bf6-ba32-94c3c8f6bf31',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fexplora.mp4?alt=media&token=bba61a77-f216-4bf6-ba32-94c3c8f6bf31',
		label:
			'La sección "Explora" de nuestro canal ofrece un vistazo fascinante a destinos desconocidos, culturas diversas y fenómenos naturales impresionantes. A través de documentales vibrantes y reportajes llenos de aventura, invitamos a los espectadores a descubrir lo extraordinario que se esconde en nuestro mundo, todo desde la comodidad de su hogar.',
		icon: '/icons/icon-explora.png'
	},
	cultura: {
		title: 'Cultura y Arte',
		bgColor: '#FB9285',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fentretenimiento.mp4?alt=media&token=68d38d32-1be7-4365-af3e-45bec8ccfa81',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fentretenimiento.mp4?alt=media&token=68d38d32-1be7-4365-af3e-45bec8ccfa81',
		label:
			'La sección de "Cultura y Arte" de nuestro canal trae lo mejor de la música, cine, series y celebridades. Con entrevistas exclusivas, reseñas de los estrenos más esperados y lo más destacado de la cultura pop, mantenemos a los espectadores al tanto de lo más divertido y emocionante del mundo del entretenimiento.',
		icon: '/icons/icon-entretenimiento.png'
	},
	naturaleza: {
		title: 'Naturaleza',
		bgColor: '#5A774F',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fnaturaleza.mp4?alt=media&token=62bb9d59-8e3a-4c90-baf3-0f1a390c7821',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fnaturaleza.mp4?alt=media&token=62bb9d59-8e3a-4c90-baf3-0f1a390c7821',
		label:
			'La sección de "Naturaleza" de nuestro canal turístico invita a los espectadores a descubrir los paisajes más impresionantes y la biodiversidad que el mundo tiene para ofrecer. A través de recorridos visuales por bosques, montañas, playas y reservas naturales, mostramos la belleza y la importancia de conservar nuestros entornos naturales, inspirando a viajar y explorar con respeto por el planeta.',
		icon: '/icons/icon-naturaleza.png'
	},
	historia: {
		title: 'Historia',
		bgColor: '#515A80',
		mainVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fhistoria.mp4?alt=media&token=915d4d9b-8b71-423a-bded-82695bd2b3f3',
		altVideoSrc:
			'https://firebasestorage.googleapis.com/v0/b/laif-dc6f1.appspot.com/o/videos%2Fhistoria.mp4?alt=media&token=915d4d9b-8b71-423a-bded-82695bd2b3f3',
		label:
			'La sección de "Historia" de nuestro canal turístico transporta a los espectadores a través del tiempo, explorando los eventos y monumentos que han marcado el legado de diversas culturas. Con relatos fascinantes y recorridos por sitios históricos, esta sección ofrece una mirada profunda a las raíces del mundo, conectando el pasado con los destinos que hoy podemos visitar',
		icon: '/icons/icon-historia.png'
	}
};